# SCADA Load Forecast Offline PWA LV9.11

Bản LV9.11 phát triển từ lõi ổn định LV8.9/LV8.6, bổ sung giao diện **Workflow / Workbench** để người vận hành thao tác theo từng luồng, chỉ hiển thị bước đang làm thay vì toàn bộ mục dọc.
...
- Mỗi luồng có thanh bước riêng, nút **Quay lại / Tiếp tục / Thực hiện chính**. Từ LV9.9, khung chức năng của bước hiện tại được đưa trực tiếp vào vùng làm việc bên phải, không còn nằm dưới shell workflow.
...
LV9.9 không thay đổi thuật toán lõi của LV8.9. Giao diện Workflow chỉ ẩn/hiện các khung chức năng phù hợp với bước hiện tại. Nếu cần truy cập mọi nút cùng lúc, bật **Hiển thị tất cả mục**.

## Cách dùng nhanh

1. Mở ứng dụng và chọn Luồng A/S/B/C.
2. Bấm từng bước ở thanh bên trái hoặc dùng **Quay lại / Tiếp tục**.
3. Ở mỗi bước, đọc hướng dẫn trong khung LV9.9 rồi thao tác trên khung chức năng đang hiển thị.
4. Có thể bấm **Thực hiện chính** để kích hoạt nút chính của bước hiện tại.
5. Khi cần kiểm tra toàn bộ, bật **Chế độ chuyên gia**.

## Cập nhật PWA

Sau khi chép bản mới vào máy, bấm **Ép cập nhật bản mới**, sau đó đóng/mở lại ứng dụng để chắc chắn cache cũ đã được thay bằng LV9.9.
...
## LV9.9.1 - Phân luồng thật

Bản LV9.9 ban đầu chỉ thêm thanh workflow nhưng thiếu CSS ẩn các khu vực cũ nên giao diện vẫn giống dạng nhiều mục dọc. LV9.9.1 sửa lại theo đúng thiết kế Workbench:
...
## LV9.9 - Workbench đúng nghĩa

LV9.1 đã phân luồng thật nhưng khung thao tác vẫn nằm bên dưới shell workflow, làm người vận hành phải kéo xuống và mất không gian. LV9.9 sửa bố cục:
...
- Service Worker cache đổi sang LV9.9 và cache đúng file `workflow_lv9.js`.


## LV9.9 - Tối ưu không gian Workbench
...
## LV9.9 - Tách phần dùng chung của Luồng B và Luồng C

Bản LV9.9 xử lý nhược điểm Luồng B và Luồng C có nhiều bước giống nhau bằng cách thêm **Luồng S - Chuẩn bị chung cho B/C**.
...
## LV9.9 - Tách Mục 10 theo bước Workflow

LV9.9 tách khối Mục 10 cũ thành nhiều khung riêng để phù hợp với từng bước của luồng công việc:
...
## LV9.9 - Làm rõ Luồng B2/B3/B4/B5

LV9.9 tách khối dự báo đa cấp 10C thành bốn khung nhỏ để người vận hành không bị lẫn thao tác:
...
## LV9.9 - Sửa B2 bị ẩn Mục 10B

Bản LV9.9 sửa lỗi trong Workbench Luồng B: khi vào B2, khung 10B bị ẩn nên người vận hành không thấy các nút **Dự báo trạm/lộ đang chọn** và **Dự báo tất cả trạm/lộ**.
...
Sửa lỗi nhận diện khung `10B) B1 - Dự báo nhanh bằng model vận hành` trong Workflow: trước đó biểu thức nhận diện chỉ khớp `10B) Dự báo nhanh`, nên B1 có thể không đưa khung 10B vào Workbench. LV9.9 nhận diện đúng 10B ở B1 và vẫn giữ 10B trong B2.


## LV9.9 - Xuất báo cáo HTML offline

Bổ sung Mục 10H để xuất các báo cáo dạng HTML độc lập, mở được offline:

- `forecast_report_lv9_9.html`: bảng forecast chi tiết và đồ thị P dự báo.
- `forecast_summary_report_lv9_9.html`: báo cáo đa cấp, Pmax, giờ Pmax, sản lượng và ca vận hành.
- `forecast_evaluation_report_lv9_9.html`: sai số từng mốc, MAE, MAPE, RMSE, BIAS.
- `calibration_report_lv9_9.html`: bảng hiệu chỉnh model và so sánh trước/sau hiệu chỉnh nếu có.
- `scada_forecast_full_report_lv9_9.html`: báo cáo tổng hợp gồm dự báo, summary, đánh giá, hiệu chỉnh và nhật ký thao tác.

Trong Workflow, Luồng B5 và C6 sẽ hiển thị thêm Mục 10H để xuất báo cáo HTML. Các báo cáo không cần CDN, không cần internet và có nút In / Lưu PDF bằng chức năng in của trình duyệt.


## LV9.11 - Mở báo cáo HTML đã xuất trực tiếp trong chương trình

LV9.11 bổ sung trong khối **10H**:

- Chọn file `.html/.htm` đã xuất rồi mở ngay trong PWA.
- Xem lại báo cáo vừa xuất trong phiên làm việc hiện tại.
- Mở báo cáo đang xem trong tab mới.
- In / lưu PDF báo cáo đang xem.
- Đóng khung xem để quay lại thao tác dự báo.

Chức năng này không thay đổi lõi dự báo/huấn luyện. Báo cáo vẫn là file HTML độc lập, mở offline được.

## LV9.11 - Sao lưu phiên, kiểm tra model và mô phỏng kịch bản

LV9.11 bổ sung ba khối mới:

### 10I - Sao lưu và phục hồi phiên làm việc

- Xuất toàn bộ phiên làm việc ra `session_lv9_11_*.json`.
- Phục hồi lại phiên từ file JSON đã xuất.
- Lưu nhanh phiên vào trình duyệt offline và nạp lại khi cần.
- Nội dung phiên gồm dữ liệu RAM, ánh xạ cột, model, forecast, kết quả đánh giá, hiệu chỉnh và trạng thái liên quan.

### 10J - Cảnh báo chất lượng model

- Kiểm tra model đã nạp có đúng chữ ký vận hành hay không.
- Kiểm tra chỉ danh trong dữ liệu mới có nằm trong phạm vi model đã học hay không.
- Cảnh báo dữ liệu RAM có quá nhiều dòng forecast chèn vào RAM.
- Cảnh báo P=0 chưa có cờ vận hành.
- Cảnh báo trộn chu kỳ dữ liệu khác nhau.
- Xuất `model_quality_lv9_11.csv`.

### 10K - Mô phỏng kịch bản dự báo

- Mô phỏng tăng/giảm nhiệt độ.
- Mô phỏng tăng/giảm phụ tải theo phần trăm.
- Mô phỏng chuyển tải từ chỉ danh này sang chỉ danh khác.
- Mô phỏng giảm tải/mất điện theo chỉ danh.
- Xuất `scenario_forecast_lv9_11.csv`.
- Có thể dùng kết quả mô phỏng làm forecast hiện tại trong RAM để tiếp tục xem báo cáo/cảnh báo, nhưng không sửa model và không sửa dữ liệu gốc.
